...
if is_l1:
    DEVICE = 'cpu'
    RUN_NAME = 'segnet_pretrained_sigmoid_unfrozen_w_nobn_run_l1Loss_006'
    lossfn = L1Loss()

else:
    DEVICE = 'cpu'
    RUN_NAME = 'segnet_pretrained_sigmoid_unfrozen_w_nobn_run_MSELoss_006'
    lossfn = MSELoss()


net = MapModelPretrainedVGG(use_batch_norm=False, freeze_encoder=True)

if os.path.exists(f'models/{net.__class__.__name__}/{RUN_NAME}'):
    if not os.listdir(f'models/{net.__class__.__name__}/{RUN_NAME}') == []:
...
optimizer = Adam(net.parameters(), lr=3e-7)
